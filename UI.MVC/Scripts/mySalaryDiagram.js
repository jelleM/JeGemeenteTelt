$(document).ready(function () {
    //Bereken gemeentebelasting
    var bruto = 0;
    var verschil1 = 8709.99;
    var verschil2 = 3689;
    var verschil3 = 8259;
    var verschil4 = 17209;
    var max1 = 8710;
    var max2 = 12400;
    var max3 = 20660;
    var berekeningschijf1 = 0;
    var berekeningschijf2 = 0;
    var berekeningschijf3 = 0;
    var berekeningschijf4 = 0;
    var brutoMinRSZ = 0;
    var inkomenschijfsom = 0;
    var belastingsvrij = 0;
    var personenbelasting = 0;
    var gemeentebelasting = 0;
    var jaarsal = 0;
    var type = $("#worktypehidden").text();
    var citytax = parseFloat($("#cityTax").text().replace(',', '.'))
    jaarsal = $("#salary").val() * 12;
    if (type == "Arbeider") {
        bruto = (jaarsal * 1.08) * 0.1307;
    }
    else if (type == "Bediende") {
        bruto = jaarsal * 0.1307;
    }
    brutoMinRSZ = jaarsal - bruto;
    ...
    gemeentebelasting = (personenbelasting * citytax) / 12;
    ...
    var arr = [];
    var city = $("#city").text();
    var tax = parseFloat($("#cityTax").text().replace(',', '.'));
    ...
    $.getJSON("../Api/Budget/" + city, function (data) {
        ...
        function stylingBedrag(bedrag) {
            if ($("#salary").val() != null) {
                if (bedrag == arr[3]) {
                    bedrag = gemeentebelasting;
                } else {
                    //bedrag = bedrag / arr[3] * $("#salary").val().replace(",", ".") * tax;
                    bedrag = bedrag / arr[3] * gemeentebelasting;
                }
            }
            return ("€ " + ((bedrag * 1000) / 1000).toFixed(2)).replace(".", ",");
        }
        ...
        function makeArc(input, radius, innerRadius) {
            ...
            if (d3.select("#budgetchart").select("h4").empty()) {
                d3.select("#budgetchart").append("h4").text("Totale uitgaven: " + stylingBedrag(arr[3])).attr("id", "BudgetText");
