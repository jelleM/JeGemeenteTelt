$(document).ready(function () {
    ...
    var w = 750, //width
        ...
        tempTitle = [],
        tempChange = [], // tijdelijk <p> waarde opslagen (maakt het mogelijk om aangeklikte waarde terug te laten zien na hover)
    ...
    // JSON (json to array) 
    var arr = [];
    var proposal = $("#proposal-id").text();

    $.getJSON("../Api/BudgetProposal/" + proposal, function (data) {
        ...
        var totaalBedrag = 0;
        var tempArr = arrayMaker(arr[14], null);
        for (var obj in tempArr ) {
            totaalBedrag += tempArr[obj].CategorieInformatie.Bedrag;
        }
        ...
        function arrayMaker(a, x) {
            ...
            for (var obj in a) {
                if (x == null) {
                    if (a[obj].CategorieInformatie.Categorie.CategorieNiveau == 1) {
                        arrNew.push(a[obj]);
                    }
                } else {
                    for (var subcat in x.CategorieInformatie.Categorie.SubCategorieen) {
                        if (x.CategorieInformatie.Categorie.SubCategorieen[subcat].CategorieId == a[obj].CategorieInformatie.Categorie.CategorieId) {
                            arrNew.push(a[obj]);
                        }
                    }
                }
        ...
        function makeArc(input, radius, innerRadius) {
            ...
            if (d3.select("#budgetchart").select("h4").empty()) {
                d3.select("#budgetchart").append("h4").text("Totale uitgaven: " + stylingBedrag(totaalBedrag)).attr("id", "BudgetText").attr("class","text-center");
                tempTitle.push("Totale uitgaven: " + stylingBedrag(totaalBedrag));
            }
            if (d3.select("#budgetchart").select("h5").empty()) {
                var bedrag = 0;
                if (arr[12] != null) {
                    bedrag = arr[12].Bedrag - arr[7];
                } else {
                    bedrag = arr[7];
                }
                d3.select("#budgetchart").append("h5").text(stylingBedrag(bedrag)).attr("id", "BudgetChangeText").attr("class", "text-center");
                tempChange.push(stylingBedrag(arr[7]));
            }
            ...
            function mouseover(d, i) {
                if (d3.select(this.parentNode).attr("class") == "open") { //enkel hoverbaar als klasse open.
                    ...
                    d3.select("#budgetchart").select("h4").text(input[i].CategorieInformatie.Categorie.Naam + ": " + stylingBedrag(input[i].Bedrag));
                    d3.select("#budgetchart").select("h5").text(stylingBedrag(input[i].Bedrag - input[i].CategorieInformatie.Bedrag));
                    //d3.select("#BudgetChangeText").text(stylingBedrag(arr[7] + arr[12].Bedrag));
            ...
            function mouseout(d) {
                ...
                d3.select("#budgetchart").select("h5").text(tempChange[tempChange.length - 1]); //<p> terug zetten naar de tempTitle
                var bedrag = 0;
                if (arr[12] != null) {
                    bedrag = arr[12].Bedrag - arr[7]
                } else {
                ...
                d3.select("#BudgetChangeText").text(stylingBedrag(bedrag));
            }
            ...
            function click(d, i) {
                if (d3.select(this.parentNode).attr("class") == "open") { // enkel klasse open is klikbaar
                    ...
                    tempChange.push(d3.select("#budgetchart").select("h5").text()); // waarde geven aan temptitle
                    d3.selectAll("g").attr("class", "closed"); // klasse op closed zetten
                    if (d3.selectAll("g").size() < 3) {
                        makeArc(arrayMaker(arr[14], input[i]), radius * 1.2, radius);
                    }
            ...
            function circleclick(d) {
                if (d3.selectAll("g")[0].length > 1) { // als er > 1 g-element is kan er een g-element worden verwijdert
                    ...
                    d3.select("#budgetchart").select("h5").text(tempChange.pop());
                    d3.select("#budgetchart").select("h5").text(tempChange[tempChange.length - 1]); //<p> terug zetten naar de tempTitle
                ...
                if (d3.selectAll("g")[0].length < 2) {
                    d3.select("#BudgetChangeText").text(stylingBedrag(arr[12].Bedrag - arr[7]));
        ...
        if (arr.length != 0) {
            makeArc(arrayMaker(arr[14], null), r, ir); // children selecteren via [1] (kan beter!?)
        } else {
